...
getJson <- function(variableSelected,blList, wlList) {
  ...
  names(Data_0) = Mapping$New_Variable[match(names(Data_0), 
        Mapping$Old_Variable)]
  ...
  ## Black Listing
  BL <- matrix(c(myVector_BL),ncol = 2, byrow = TRUE)

  # White Listing
  WL <- matrix(c(myVector_WL),ncol = 2, byrow = TRUE)
